...
void debug_interrupt_handler(void) {
  ...
  if(zf_debug_init_flag){   
        ...
        if(fifo_data_count != 0){                                                // 读取到数据了
            fifo_read_buffer(&debug_uart_fifo, uart_buffer_s[0], &fifo_data_count, FIFO_READ_AND_CLEAN);    // 将 fifo 中数据读出并清空 fifo 挂载的缓冲
            uart_write_string(DEBUG_UART_INDEX, "\r\nUART get data:");                // 输出测试信息
            uart_write_buffer(DEBUG_UART_INDEX, uart_buffer_s[0], fifo_data_count);   // 将读取到的数据发送出去
        }
    }
}
