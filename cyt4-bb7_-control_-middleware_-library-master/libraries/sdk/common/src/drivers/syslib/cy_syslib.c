...
void Cy_SysLib_Delay(uint32_t milliseconds) {
  while (milliseconds > CY_DELAY_MS_OVERFLOW) {
    /* This loop prevents an overflow in value passed to Cy_SysLib_DelayCycles()
     * API. At 100 MHz, (milliseconds * cy_delayFreqKhz) product overflows in
     * case if milliseconds parameter is more than 42 seconds.
     */
...
// rmkn: to be checked
  ...
  uniqueId = ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_MINOR.u8Register
              << CY_UNIQUE_ID_DIE_MINOR_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_SORT.u8Register
              << CY_UNIQUE_ID_DIE_SORT_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_Y.u8Register
              << CY_UNIQUE_ID_DIE_Y_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_X.u8Register
              << CY_UNIQUE_ID_DIE_X_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_WAFER.u8Register
              << CY_UNIQUE_ID_DIE_WAFER_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_LOT[2u].u8Register
              << CY_UNIQUE_ID_DIE_LOT_2_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_LOT[1u].u8Register
              << CY_UNIQUE_ID_DIE_LOT_1_Pos) |
             ((uint64_t)((stc_SFLASH_t *)SFLASH_BASE)->unDIE_LOT[0u].u8Register
...
static void Cy_SysLib_SetWaitStates_LP(uint32_t clkHfMHz) {
  ...
  if (clkHfMHz <= CY_SYSLIB_FLASH_LP_WS_0_FREQ_MAX) {
    ...
    defined(tviibe4m) || defined(tviice4m)
  FLASHC->unFLASH_CTL.stcField.u4MAIN_WS = waitStates;
#elif defined(tviibh4m) || defined(tviibh8m) || defined(tviic2d6m) ||          \
    defined(tviic2d4m) || defined(tviic2d6mddr) || defined(tviibh16m)
...
#if (CY_CPU_CORTEX_M7) && (CY_SYSLIB_CORE_D_CACHE_MAINTENANCE ==               \
                           CY_SYSLIB_CORE_D_CACHE_MAINTENANCE_ON)
