...
void oled_show_int(uint16 x, uint16 y, const int32 dat,
                   uint8 num); // OLED 显示32位有符号 (去除整数部分无效的0)
void oled_show_uint(uint16 x, uint16 y, const uint32 dat,
                    uint8 num); // OLED 显示32位无符号 (去除整数部分无效的0)
void oled_show_float(uint16 x, uint16 y, const double dat, uint8 num,
                     uint8 pointnum);

void oled_show_binary_image(
    uint16 x, uint16 y, const uint8 *image, uint16 width, uint16 height,
    uint16 dis_width,
...
void oled_show_wave(uint16 x, uint16 y, const uint16 *image, uint16 width,
                    uint16 value_max, uint16 dis_width,
                    uint16 dis_value_max); // OLED 显示波形
void oled_show_chinese(uint16 x, uint16 y, uint8 size,
                       const uint8 *chinese_buffer,
                       uint8 number); // OLED 汉字显示
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
#define oled_displayimage7725(p)                                               \
  (oled_show_binary_image(0, 0, (p), OV7725_W, OV7725_H, 128, 64))
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
#define oled_displayimage03x(p, x)                                             \
  (oled_show_gray_image(0, 0, (p), MT9V03X_W, MT9V03X_H, 128, 64, (x)))
//===================================================定义 OLED
// 扩展配置=================================================
