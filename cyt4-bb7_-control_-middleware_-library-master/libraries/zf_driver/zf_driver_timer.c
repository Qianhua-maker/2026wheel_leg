...
void timer_start(timer_index_enum index) {
  Cy_Tcpwm_Counter_Enable(
      (volatile stc_TCPWM_GRP_CNT_t *)&TCPWM0->GRP[2].CNT[index + 3]);
...
uint32 timer_get(timer_index_enum index) {
  return ((Cy_Tcpwm_Counter_GetCounter(
              (volatile stc_TCPWM_GRP_CNT_t *)&TCPWM0->GRP[2].CNT[index + 3])) /
          (driver_timer_mode[index] == TIMER_MS ? 8000 : 8));
}
...
void timer_clear(timer_index_enum index) {
  Cy_Tcpwm_Counter_SetCounter(
      (volatile stc_TCPWM_GRP_CNT_t *)&TCPWM0->GRP[2].CNT[index + 3], 0);
}
 ...
 * @return uint32_t
 */
uint32_t GetSystemTimer(void) { return timer_get(TC_TIME2_CH1); }