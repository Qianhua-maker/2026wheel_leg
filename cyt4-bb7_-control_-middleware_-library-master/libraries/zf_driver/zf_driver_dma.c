 ...
 */
/*********************************************************************************************************************
 * CYT4BB7CEE Opensourec Library 即（CYT4BB7CEE 开源库）是一个基于官方 SDK
 *接口的第三方开源库 Copyright (c) 2023 SEEKFREE 逐飞科技
 *
...
// dma_channel_enum 定义) 返回参数     void 使用示例     dma_disable(); 备注信息
//-------------------------------------------------------------------------------------------------------------------
void dma_disable(void) {
  //	Cy_DMA_Channel_Disable(DW0, dma_obj.resource.channel_num); //
  // 失能DMA模块
}
...
// 函数简介     dma 传输使能
// 参数说明     ch              选择 dma 通道 (详见 zf_driver_dma.h 中枚举
// dma_channel_enum 定义) 返回参数     void 使用示例     dma_enable(); 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明      dma_count           设置dma搬移次数
// 返回参数      uint8
// 使用示例      dma_init(MT9V03X_DATA_ADD, mt9v03x_image[0], MT9V03X_PCLK_PIN,
// EXTI_TRIGGER_RISING, MT9V03X_IMAGE_SIZE); 备注信息
//-------------------------------------------------------------------------------------------------------------------
void dma_init(uint8 *source_addr, uint8 *destination_addr,
              gpio_pin_enum exti_pin, exti_trigger_enum trigger,
              uint16 dma_count) {
  // set up the descriptor, provide the configuration parameters for the
  // descriptor
  // cy_stc_pdma_descr_config_t descriptor;
