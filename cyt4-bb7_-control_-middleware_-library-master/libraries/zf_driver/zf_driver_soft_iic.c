...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
uint8_t soft_iic_write_8bit_registers (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, const uint8 *data, uint32 len)
{
    zf_assert(NULL != soft_iic_obj);
    zf_assert(NULL != data);
    soft_iic_start(soft_iic_obj);
    if(soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1)!=1){ // 假如应答信号为空，则停止iic并返回1（表示失败
      soft_iic_stop(soft_iic_obj);
      return 1;
    } 
    soft_iic_send_data(soft_iic_obj, register_name); // 发送寄存器地址并等待应答
    while(len --)
    {
        if(soft_iic_send_data(soft_iic_obj, *data ++)!=1){ // 假如应答信号为空，则停止iic并返回1（表示失败
          soft_iic_stop(soft_iic_obj);
          return 1;
        } 
    }
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
uint8_t soft_iic_read_8bit_registers (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, uint8 *data, uint32 len)
{
    zf_assert(NULL != soft_iic_obj);
    zf_assert(NULL != data);
    soft_iic_start(soft_iic_obj);
    if(soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1)!=1){ // 假如等待应答失败，返回1（表示失败
       soft_iic_stop(soft_iic_obj);
       return 1;
    }
    soft_iic_send_data(soft_iic_obj, register_name);
